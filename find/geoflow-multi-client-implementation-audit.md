...
本阶段已拆为独立执行计划：[geoflow-multi-client-phase-6-execution.md](D:/GEOFlow-2.3.0/find/geoflow-multi-client-phase-6-execution.md)。执行顺序为 `6A-0` 普通运营自助创建客户/项目 → `6A-1` 现有资料后台流程与严格隔离 → 阶段 3/4 发布门闸与批次合同 → `6A-2` 资料→任务→文章→发布批次回归 → `6B/6C` 配额、队列公平性、告警与报表加固。
