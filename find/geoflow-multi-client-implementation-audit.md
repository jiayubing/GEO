...
- `operator`：只能访问已分配或本人创建并已建立成员关系的项目；可以自助创建客户主体和项目，并在项目内上传资料、维护库、创建任务、审核文章、提交批次；不能批准平台发布。管理员给运营人员分配项目暂不作为本阶段实现目标。
- 客户不是 GEOFlow 的系统用户。客户资料如何交付给运营人员不属于本计划；运营人员可以先自助创建项目，再在该项目上下文中创建和上传知识库、关键词库、标题库、图片库等资料，GEOFlow 只负责项目归属、资料事实和处理审计。

角色值、迁移、创建/撤销权限和成员唯一性必须在阶段 1 落地。一个管理员可以属于多个项目，但每个项目的角色必须是显式成员记录，不通过 `admins.role` 推断项目访问。

### 4.2 登录后的项目切换

运营人员仍然使用一个员工账号。登录后从自己被分配或本人创建的项目中选择或切换当前项目；创建项目成功后自动切换到该项目上下文。这不是“一个账号绑定一个客户”，也不是把项目 ID 当作安全凭证。
...
本阶段已拆为独立执行计划：[geoflow-multi-client-phase-6-execution.md](D:/GEOFlow-2.3.0/find/geoflow-multi-client-phase-6-execution.md)。执行顺序为 `6A-0` 普通运营自助创建客户/项目 → `6A-1` 现有资料后台流程与严格隔离 → 阶段 3/4 发布门闸与批次合同 → `6A-2` 资料→任务→文章→发布批次回归 → `6B/6C` 配额、队列公平性、告警与报表加固 → `6D` 试点反馈的审批与运营流程修订。

**实施状态（2026-08-24）**

- 6A-0、6A-1、6A-2、6B、6C 与 6D-1/6D-2/6D-3 已完成代码和 focused 回归。6D-3 将一个文章创作任务建模为该任务唯一的发布批次：平台项目只按间隔生成草稿，审核通过的文章归集到任务 batch，达到文章上限且审核结论齐备时或暂停任务时自动提交平台审批；不自动公开或写入远程渠道。
- 6D-1 已把超级管理员固定为平台全局/无项目上下文：顶部不加载运营项目，旧项目 session 会被清除，后端拒绝超级管理员调用项目切换接口；普通管理员仍只看到 active membership 项目。
- 6D-2 新增与项目运营页隔离的平台审批中心：只有 super_admin 无需项目上下文即可查看、处理已提交批次，列表显示客户、项目、提交人、状态和时间；项目内普通批次页仍按 membership 过滤，审批动作继续由批次服务校验超级管理员身份、项目状态和状态机条件。
- 6D-1 focused 与直接调用方回归为 92 tests、561 assertions 全通过；全库运行结果为 1418 passed、11099 assertions、51 failed、1 skipped，剩余失败属于已登记的旧 authorization fixture、容器缺少 `rsync` 和无关首页内容断言。

**阶段总目标**

- 在项目边界稳定后，先让普通运营人员能够自助建立客户/项目并完成项目资料准备，再沿用现有后台完成“资料 → 任务 → 文章 → 发布批次”流程，最后补充项目级用量审计、配额和队列公平性；不复制管理员账号，也不让客户进入 GEOFlow。

**共同边界**

- 客户资料的外部交付方式不属于 GEOFlow 改造范围；不新增外部接收层、客户 webhook、客户上传 API、客户 token 或客户导入身份。
- 管理员给运营人员分配项目暂不实现；本阶段只实现运营人员自助创建项目并自动加入该项目。后续如需转移/分配成员，应另立权限需求和验证计划。

#### 6A-0：普通运营人员自助创建客户/项目

**目标**

- 普通 `operator` 在后台填写客户名称和项目名称，创建客户主体与项目，并自动成为该项目的 active `operator`。
- 创建成功后自动写入项目上下文，使运营人员可以立即进入该项目继续维护资料和创建任务。

**边界**

- `Client`、`ClientProject`、`ClientProjectMember` 必须在一个事务中创建；客户和项目不是登录主体，不能借此创建客户账号或客户 token。
- 项目 slug/名称的唯一性、重复提交幂等和失败回滚必须由服务层与数据库约束共同保证；不得信任请求体传入的 `project_id` 来决定归属。
- 创建入口只允许普通运营人员为自己创建新的客户/项目；跨项目批量创建、替他人创建成员关系和管理员分配项目不在本子阶段。

**最小验证**

- 普通运营人员可成功创建客户+项目，并存在唯一 active `operator` 成员关系；创建失败时三类记录全部回滚。
- 重复点击/重试不会产生重复客户、项目或成员；slug 冲突返回可理解的校验错误。
- 创建者自动切换到新项目上下文；其他未授权运营人员无法读取或修改该项目，且不能通过 URL 或请求体伪造项目归属。

#### 6A-1：项目内资料创建/上传与严格隔离

**目标**

- 在 6A-0 创建的项目上下文中，运营人员继续使用现有后台页面和 Service 创建/上传知识库、关键词库、标题库、图片库等文章创作前资料。
- 资料、作者、分类及其引用始终继承当前项目 owner，并由同一套项目授权约束保护。

**边界**

- 不改变现有资料录入方式；未来如需批量导入，应另立需求和验证计划，不在本次多客户隔离改造中顺手引入。
- 未完成项目过滤的页面不得向普通 `operator` 开放；前端隐藏菜单不能代替后端授权。

**最小验证**

- 运营人员在同一项目下通过现有后台流程创建/上传资料成功；切换项目后不能看到或修改其他项目的库和条目。
- 资料详情、更新、删除、导入、批量操作和 API 均按项目授权；跨项目引用在服务层和持久化不变量层被拒绝。
- 资料的项目 owner、操作者、重复提交和失败结果可审计，日志不泄露客户正文或密钥。

#### 6A-2：资料 → 任务 → 文章 → 发布批次全流程回归

**目标**

- 验证普通运营人员从自建项目开始，完成资料准备、任务创建、文章生成/审核并提交发布批次；超级管理员仍在批次页面审批和执行发布。

**边界**

- 复用阶段 2、3、4 已冻结的任务、文章和批次状态机，不在本子阶段新增旁路发布入口或改变平台审批闸门。
- 只做同项目、内部测试和受控本站回归；阶段 5 的 Enterprise、URL Import 和中央站公开资格仍按原顺序独立执行。

**最小验证**

- 新建项目的资料只能被该项目任务引用，生成文章继承同一项目，运营人员可提交批次但不能批准/执行平台发布。
- 超级管理员可看到并处理其他运营人员提交的批次；批次快照、文章版本变化、重复提交和失败/不确定状态均可读回。
- 至少完成一条从项目创建到批次提交的验收链路，并通过跨项目拒绝和回归测试后，才允许真实内部资料试点。

#### 6B：项目级配额与存储限制

**目标**

- 基于阶段 2 已建立的不可变 `ai_usage_events`/observation，实施项目 quota、存储、文章数和并发限制。

**边界**

- 事件继续记录 project、model、operation、attempt、units、outcome 和 reservation 生命周期；平台级 AI Key、模型和敏感词基线仍由超级管理员管理。
- 不把 project ID、quota 或 secret 放进可篡改 payload。

**最小验证**

- AI 成功、失败、fallback、重试和不确定调用均产生可追溯 usage observation；共享模型 quota 的消耗可按项目审计。
- 项目级并发/配额拒绝具备最小测试，并能在不破坏既有项目状态的情况下恢复或重试。

#### 6C：队列公平性、告警和聚合报表

**目标**

- 队列优先级、失败告警和审计报表按项目聚合，避免单个项目占满共享 worker。

**边界**

- 只改造项目级队列调度、标签、限流、告警和查询聚合；不把 Horizon 全局指标改造成客户可见看板，也不复制第二套队列状态机。
- Job 执行时仍从稳定业务 ID 解析项目 owner，不信任可篡改 payload 中的项目 ID；TaskRun/Job 状态继续由现有事实 owner 管理。
- 报表查询必须在数据库层按项目条件、owner join、索引和分页聚合；不先加载全表再由 PHP 过滤。
- 告警只面向内部管理员/运营人员；不新增客户登录、客户通知、外部资料接收、公开前台或新的渠道连接器。
- 规模化阈值、优先级和通知渠道没有产品决策时，先实现可观察性、告警事件和安全默认值，不擅自扩大为完整计费系统。

**最小验证**

- 不同项目的任务在并发和重试场景下保持可观察的公平性；失败告警、审计查询和聚合报表可按项目读取。
- 500 个项目聚合查询不读取全表后再在应用层过滤，并有可重复的性能/查询计划证据。
...
| 6 | 拆成 6A-0/6A-1/6A-2/6B/6C | `6A-0` 普通运营自助创建客户/项目并自动成为 operator；`6A-1` 项目内资料创建/上传与严格隔离；`6A-2` 资料→任务→文章→发布批次全流程回归；`6B` 项目 quota/存储/文章数/并发；`6C` 队列公平性、告警和聚合报表 | 中高；6A-0 是资料试点前置，6A-1/6A-2 收口真实运营链路，6B/6C 是试点后的规模化加固；不新增客户接收层 |
...
2. 阶段 2 的 `2D` 和阶段 3 的 `3A–3F` 通过前，不得让新项目进入自动公开发布或真实外部发布；阶段 6A-0/6A-1 只负责在项目上下文中准备运营资料，阶段 6A-2 必须等阶段 3/4 合同稳定后再回归资料→任务→文章→批次流程。
...
5. 阶段 6 的 `6A-0`、`6A-1`、`6A-2` 是真实多客户资料试点的前置条件；不得在 6A-0 未通过前开放普通运营的项目创建，在 6A-1 未通过前开放资料写入口，在 6A-2 未通过前接入真实内部资料。`6B/6C` 的 quota、队列公平性和规模化报表不应阻塞首个小规模试点；阶段 7 始终是可选项。
...
阶段 6A-0：普通运营自助创建客户/项目并自动成为 operator
阶段 6A-1：验证运营人员在项目上下文中创建/上传资料并保持严格隔离
...
1. 运营人员只能看到已分配或本人创建项目的知识库、关键词库、标题库、图片库、任务和文章。
...
24. 普通运营人员创建客户+项目时，客户、项目和 active `operator` 成员关系在同一事务中落库；重复提交幂等，创建者自动进入项目上下文，且不能伪造其他项目归属。
...
建议的首个实现提交只包含阶段 1 的项目基础/迁移合同，以及阶段 2 的知识库 → 任务 → 文章隔离所需的最小代码和测试；不得同时引入发布批次、批量导入、项目官网或无关格式化。阶段 2、阶段 6A-0/6A-1/6A-2 和阶段 3 的最小验证通过后，才允许接入真实试点资料；阶段 6B/6C 不应被首个小规模试点反向扩大为前置范围。
...
方向可以保留，但只有在阶段 0–3 以及阶段 6A-0/6A-1/6A-2（运营人员自建项目、维护资料并回归发布批次流程）的最小验证通过后，才可以接入真实多客户试点资料；此时仅允许内部处理和审核，不允许公开发布。完成阶段 4E 后，才可以进行受控的本站发布试点。完整顺序为：
...
- 阶段 2 的跨项目拒绝和队列继承通过；
- 阶段 6A-0 的客户/项目自助创建、事务回滚、幂等和自动 operator 成员关系通过；
- 阶段 6A-1 的知识库、关键词库、标题库和图片库后台流程在项目上下文中可用，并通过跨项目拒绝；
- 阶段 6A-2 的资料 → 任务 → 文章 → 发布批次回归通过；普通运营只能提交批次，超级管理员才能批准和执行；
